...
```{r viz-score}
evals %>%
  ggplot(aes(x = score)) + geom_histogram(bins = 20)
```
...
evals %>%
  ...
  geom_point() +
  labs(
    title = "Relationship between score and bty_avg",
    x = "Score",
    y = "Bty_avg"
  )
```
...
ggplot(data = evals, aes(x = score, y = bty_avg)) + geom_point() + geom_smooth(method = "lm", se = FALSE) + labs(title = "Relationship between score and bty_avg", x = "Score", y = "Bty_avg")
```
